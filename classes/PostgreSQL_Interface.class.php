<?php
	...
	* geschlossen.
	*
	* $Log: MySQL_Interface.class.php,v $
	* Revision 1.8  2006/10/20 08:44:48  manhart
	* Improved isConnected
	*
	* Revision 1.7  2006/08/08 13:05:42  manhart
	* Fix overload in MySQL_Interface
	*
	* Revision 1.6  2006/04/11 11:57:10  manhart
	* Aenderung listfields
	*
	* Revision 1.5  2006/03/21 09:57:51  manhart
	* new foundRows()
	*
	* Revision 1.4  2005/04/04 06:26:44  manhart
	* new getClientInfo
	*
	* Revision 1.3  2004/11/09 12:03:15  manhart
	* Fix
	*
	* Revision 1.2  2004/11/09 09:10:06  manhart
	* Fix for Zend Studio "Include check"
	*
	* Revision 1.1.1.1  2004/09/21 07:49:25  manhart
	* initial import
	*
	* Revision 1.3  2004/04/13 13:16:20  manhart
	* update
	*
	* Revision 1.2  2004/04/01 15:10:30  manhart
	* Initial Import
	*
	* Revision 1.1  2004/04/01 07:29:19  manhart
	* Initial Import
	*
	*
				...
				if ($database != '') {
					...
					if (!$conid) {
						if (version_compare(phpversion(), '4.2.0') >= 0) {
							$conid = ($this->persistency) ? @mysql_pconnect($this->host[$mode], $user, $this->__get_db_pass($database, $mode), true) :
															@mysql_connect($this->host[$mode], $user, $this->__get_db_pass($database, $mode), true);
							...
							$conid = ($this->persistency) ? @mysql_pconnect($this->host[$mode], $user, $this->__get_db_pass($database, $mode)) :
															@mysql_connect($this->host[$mode], $user, $this->__get_db_pass($database, $mode));
							#echo '#connected to ' . $database . ' with mode ' . $mode . ' (conid: '.$conid.')<br>';
						}
		    ...
		    */
			...
			function query($query, $database='')
			{
				...
				if($this->sql != '') {
					...
					if(IS_TESTSERVER and $command != 'SELECT' and $command != 'SHOW' and $command != 'INSERT' and
						...
						and $command != 'DECLARE' and $command != 'OPTIMIZE') {
			...
			function escapestring($string, $database='')
			{
				if(version_compare(phpversion(), '4.3.0', '>=')) { // 20.05.09, AM, wenn das hier am Intranet fuer aktuelle Proggis fehl schlaegt, dann die Version auf '5.0.0' aendern
					return mysql_real_escape_string($string, $this->__get_db_conid($database, SQL_READ));
				}
				else {
