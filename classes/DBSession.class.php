...
class DBSession extends PoolObject
{
    ...
    function __construct(&$interfaces, $tabledefine, $sid='', $ipUserAgentRestriction=true)
    {
        ...
        $this->DAO_Session = &DAO::createDAO($interfaces, $tabledefine);
        ...
        if ($sid == '' or !$this->sid_exists($sid)) {
            ...
            $this->DAO_Session->newSession($this->sid, $this->expire, '',
                $this->RemoteAddr, $this->HttpUserAgent);
        }
        else {
            $this->sid = $sid;
    ...
    function initialize()
    {
        ...
        if ($data === false) {
            $bResult = $this->raiseError(__FILE__, __LINE__, 'Foul! DBSession data has the value "false". '.
    ...
    */
     ...
     **/
    function setFielddata($fielddata=array())
    {
