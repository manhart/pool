		...
		class CISAM_DAO extends DAO
		{
			...
			function update($data)
			{
				...
				foreach ($data as $key => $val) {
					array_push($set, array($key, 'equal', $val));
				}
				...
				foreach ($pk as $key => $val) {
					array_push($where, array($key, 'equal', $val));
				}
...
/* -------------------------- */
	...
	class CustomCISAM_DAO extends CISAM_DAO
	{
		...
		/**
		 * CustomCISAM_DAO::CustomCISAM_DAO()
		 *
		 * Konstruktor
		 *
		 * Sets up the object.
		 ...
		 **/
		function CustomCISAM_DAO(& $client, $program, $autoload_fields=true)
		{
			$this -> client = & $client;
