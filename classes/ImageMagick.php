...
class ImageMagick extends PoolObject
{
     ...
     * The basic strategy of dithering is to trade intensity resolution for spatial resolution by averaging the intensities of several neighboring pixels. Images which suffer from
     * severe contouring when reducing colors can be improved with this option. The -colors or -monochrome option is required for this option to take effect. Use +dither to turn
     * off dithering and to render PostScript without text or graphic aliasing.
     *
     * @access public
     **/
    function dither()
     ...
     * @param string $how bei keinem Wert > Standard Rotation, 'morewidth' > rotiert das Bild nur, wenn die Breite die Hoehe ueberschreitet, 'lesswidth' > rotiert das Bild nur,
     *     wenn die Breite kleiner ist als die Hoehe.
     **/
     ...
     *
     * @param string $how 'center' > schneidet ein Quadrat von der Mitte des Bildes heraus, 'left' > schneidet ein Quadrat von der linken Seite des Bildes heraus, 'right' >
     *     schneidet ein Quadrat von der rechten Seite des Bildes heraus
     **/
    function square($how = 'center')
    {
        ...
        $command =
            "{$this->imagemagickdir}/convert -crop '{$line}' '{$this->temp_dir}/tmp{$this->count}_{$this->temp_file}' '{$this->temp_dir}/tmp".++$this->count."_{$this->temp_file}'";
    ...
    {
        return "{$this->imagemagickdir}/convert -quality {$this->jpg_quality} {$command} '{$this->temp_dir}/tmp{$this->count}_{$this->temp_file}' '{$this->temp_dir}/tmp".
            ++$this->count."_{$this->temp_file}'";
    }
