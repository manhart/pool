...
* Last Modified..: $Date: 2006/10/25 10:59:37 $
...
*/
	function &_addTextPart(&$obj, $text)
	{
		...
		if (is_object($obj)) {
			...
			$Mail_mimePart = new Mail_mimePart($text, $params);
...
*/
	function &_addHtmlPart(&$obj)
	{
		...
		if (is_object($obj)) {
			...
			$Mail_mimePart = new Mail_mimePart($this->html, $params);
			return $Mail_mimePart;
		}
	}
...
*/
	function &_addMixedPart()
	{
		...
		$Mail_mimePart = new Mail_mimePart('', $params);
		return $Mail_mimePart;
	}
...
//		}
	...
	/**
	 * Verschl�sselt Betreffzeile in Unicode-Entsprechungen
	 *
	 * @param string $input
	 * @param string $charset
	 * @return string
	 */
	function _encodeSubject($input, $charset = 'ISO-8859-1')
	{
		...
		switch ($type) {
			case 'mail':
				...
				if (!empty($this->headers['Subject'])) {
					$subject = $this->_encodeSubject($this->headers['Subject'], $this->build_params['head_charset']);
...
//				echo 'return_path: ' . $this->return_path.'<br>';
//				echo pray($headers).'<br>';
//				echo 'subject:'.$subject.'<br>';
//				echo htmlspecialchars($to).'<br>';
//				echo $this->output.'<br>';
			...
			case 'smtp':
				//require_once(addEndingSlash(DIR_PUBLIC_ROOT) . 'smtp.class.php');
				//require_once(dirname(__FILE__) . '/RFC822.class.php');
