...
class HtmlMinifierTest extends TestCase
{
    ...
    {
        $html = "<div>\n  <p>hi</p>\n</div>";
        self::assertSame($html, HtmlMinifier::minify($html, HtmlMinifier::MODE_OFF));
    }

    public function testEmptyInput(): void
    {
        self::assertSame('', HtmlMinifier::minify('', HtmlMinifier::MODE_LEAN));
    }

    public function testCollapsesIndentationBetweenBlockTags(): void
    {
        $in = "<div>\n    <p>hi</p>\n</div>";
        $out = HtmlMinifier::minify($in, HtmlMinifier::MODE_LEAN);
    ...
    public function testPreservesInlineSpacing(): void
    {
        ...
        $in = '<a>foo</a> <a>bar</a>';
        self::assertSame($in, HtmlMinifier::minify($in, HtmlMinifier::MODE_LEAN));
    }
    ...
    {
        $in = "<div>\n<!-- keep me -->\n</div>";
        self::assertStringContainsString('<!-- keep me -->', HtmlMinifier::minify($in, HtmlMinifier::MODE_LEAN));
    }

    public function testFullDropsComments(): void
    {
        $in = "<div>\n<!-- drop me -->\n</div>";
        $out = HtmlMinifier::minify($in, HtmlMinifier::MODE_FULL);
