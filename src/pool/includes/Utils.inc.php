	...
	*/
	...
	function custom_tags($foo)
	{
		if (!is_array($foo)) {
			...
			foreach ($foo as $k => $v) {
				if (is_array($foo[$k])) $foo[$k] = custom_tags($foo[$k]);
				else {
	...
	function strip_custom_tags($foo)
	{
		if (!is_array($foo)) {
			...
			foreach ($foo as $k => $v) {
				if (is_array($foo[$k])) $foo[$k] = strip_custom_tags($foo[$k]);
				else {
	...
	function stripRs($foo)
	{
		if (!is_array($foo)) {
			...
			foreach ($foo as $k => $v) {
				if (is_array($foo[$k])) $foo[$k] = stripRs($foo[$k]);
	...
	function stripNs($foo)
	{
		if (!is_array($foo)) {
			...
			foreach ($foo as $k => $v) {
				if (is_array($foo[$k])) $foo[$k] = stripNs($foo[$k]);
...
function isEmptyObject($obj)
{
	...
	function magicInfo($file, $line, $function, $class, $specific=array()) {
		if(!is_array($specific)) if(!is_null($specific)) $specific=array($specific); else $specific=array();
...
/**
	...
	function isFeiertag ($intTag, $intMonat, $intJahr) {
		...
		if ($intTag==1 AND $intMonat==1)     { return true; /*return "Neujahr";*/ }
		if ($intTag==6 AND $intMonat==1)     { return true; /*return "Dreik�nigstag";*/ }
		if ($intTag==1 AND $intMonat==5)     { return true; /*return "1. Mai";*/ }
		if ($intTag==15 AND $intMonat==8)    { return true; /*return "Maria Himmelfahrt";*/ }
		if ($intTag==3 AND $intMonat==10)    { return true; /*return "Tag der deutschen Einheit";*/ }
		if ($intTag==1 AND $intMonat==11)    { return true; /*return "Allerheiligen";*/ }
		if ($intTag==25 AND $intMonat==12)   { return true; /*return "1. Weihnachtstag";*/ }
		if ($intTag==26 AND $intMonat==12)   { return true; /*return "2. Weihnachtstag";*/ }
	...
	function isHalbfeiertag ($intTag, $intMonat, $intJahr) {
		if ($intTag==24 AND $intMonat==12) { return true; /*return "Heiligabend";*/ }
		if ($intTag==31 AND $intMonat==12) { return true; /*return "Silvester";*/ }
		return false;
	}
...
/**
 * Entschlüsselt Daten, welche vorher mit @see encryptTwofish verschlüsselt wurden.
 *
 * @param string $encryptedData Binärer string mit verschlüsselten Daten.
...
{
    return array_filter($array, function($key) use ($column) { return ($key == $column); }, ARRAY_FILTER_USE_KEY);
}
